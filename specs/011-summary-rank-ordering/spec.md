...
- Q: In WinnersCourt, how should players be ordered within each court outcome pair when assigning ranks? → A: Order alphabetically by display name within winners and within losers.
...
**Independent Test**: Complete events for Mexicano, WinnersCourt, and BeatTheBox and verify rank column values and row order exactly match mode-specific ranking rules.
...
2. **Given** a final WinnersCourt summary, **When** the table is shown, **Then** rank and row order follow final-round court-priority outcome ordering from highest court to lowest.
...
2. **Given** a final WinnersCourt summary, **When** ranking is shown, **Then** crowns still mark highest-court final-match winners.
...
- When final-round match data is incomplete for WinnersCourt court-priority ordering, summary remains available and applies deterministic fallback ordering without crashing.
...
- When events have varying court counts, WinnersCourt and BeatTheBox ordering still applies correctly from highest court group to lowest.
...
- WinnersCourt intra-pair player order uses alphabetical display-name ordering.
...
- **FR-006**: System MUST rank and order WinnersCourt final summaries by final-round court priority outcomes: highest-court winners first, then highest-court losers, continuing court-by-court in descending court number.
- **FR-006a**: System MUST order players alphabetically by display name within each WinnersCourt winners pair and within each WinnersCourt losers pair for deterministic ranking.
- **FR-007**: System MUST keep WinnersCourt row order exactly aligned with displayed rank order.
...
- **Mode Ordering Rule**: Deterministic ruleset for row order and rank assignment by event mode (Mexicano, WinnersCourt, BeatTheBox).
- **WinnersCourt Court Outcome Group**: Ordered set derived from final-round matches where each court contributes winner and loser rank slots in descending court priority.
...
- **SC-003**: In 100% of WinnersCourt final-summary fixtures, row order and displayed ranks match the final-round highest-to-lowest-court winner/loser sequence.
