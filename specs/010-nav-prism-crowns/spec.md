...
- Q: How should WinnersCourt final-highest-court draws be handled for crown assignment? → A: Not applicable; WinnersCourt winner inputs do not include draws.
...
2. **Given** a completed WinnersCourt event final summary, **When** standings are shown, **Then** both players from the winning team of the highest-court match in the final round include crown icons.
3. **Given** an in-progress event summary, **When** summary rows are shown, **Then** no crown icons are displayed.
4. **Given** a completed WinnersCourt event where highest-court winner cannot be determined, **When** final summary loads, **Then** no crown icons are shown and summary remains usable.
...
- WinnersCourt result inputs always resolve a winner and do not support draw outcomes.
...
- **FR-009**: System MUST determine WinnersCourt final winners from the final-round highest-court match winner and show crowns next to both winning player names.
- **FR-010**: System MUST show no crowns when WinnersCourt winner determination cannot be completed from available final-round highest-court data.
- **FR-010a**: System MUST treat draw-handling for WinnersCourt crown assignment as not applicable because draw outcomes are outside WinnersCourt result rules.
...
- **Winner Resolution Rule**: Deterministic rule set for deriving crowned players in final summary (all top-score ties for Mexicano; final-round highest-court winning team for WinnersCourt with no draw outcomes; none for BeatTheBox).
...
- **SC-003**: In 100% of final-summary test fixtures, crown visibility matches mode rules (Mexicano: crowns on all highest-score ties; WinnersCourt: 2 crowns from highest-court final match winner; BeatTheBox/progress: 0 crowns).
